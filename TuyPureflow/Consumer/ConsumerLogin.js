...
export default function ConsumerLogin({ navigation }) {
  ...
  const handleLogin = async () => {
    ...
    try {
      const response = await fetch('http://192.168.1.3/pureflowBackend/consumer_login.php', {
        method: 'POST',
        headers: {
  ...
  const handleForgotPassword = async () => {
    ...
    try {
      const response = await fetch('http://192.168.1.3/pureflowBackend/forgot_password.php', {
        method: 'POST',
